# Hi, I'm darils 👋

> Student focused on networking, cloud, and backend development.

## 🚀 About Me

- 💻 Learning Django, MariaDB, and cloud infrastructure
- ☁️ Working with AWS, Proxmox, and Linux servers
- 🌐 Interested in networking, virtualization, and web hosting
- 🎯 Currently improving backend and infrastructure skills

## 🛠 Tech Stack

### Languages
![Python](https://img.shields.io/badge/Python-111?style=for-the-badge&logo=python)
![SQL](https://img.shields.io/badge/SQL-111?style=for-the-badge&logo=mysql)

### Backend & Cloud
![Django](https://img.shields.io/badge/Django-111?style=for-the-badge&logo=django)
![MariaDB](https://img.shields.io/badge/MariaDB-111?style=for-the-badge&logo=mariadb)
![AWS](https://img.shields.io/badge/AWS-111?style=for-the-badge&logo=amazonaws)
![Linux](https://img.shields.io/badge/Linux-111?style=for-the-badge&logo=linux)

## 📌 Projects

### 🔹 Cloud Migration Lab
Infrastructure migration simulation using AWS EC2, Apache, CloudWatch, and RDS.

### 🔹 Home Lab Server
Self-hosted virtualization environment using Proxmox and Ubuntu containers.

### 🔹 Networking Labs
Cisco Packet Tracer VLAN, SSH, DNS, and static routing configurations.

## 📈 GitHub Stats

![Stats](https://github-readme-stats.vercel.app/api?username=PottatoWasFound&show_icons=true&theme=tokyonight)

## 📫 Contact

- Tiktok : darils
- Instagram: @darilwasfound
